...
int found_child (struct thread *child);
bool failed = 0;
int sema = 0;
...
start_process (void *file_name_)
{
  ...
  /* If load failed, quit. */
  thread_current ()->fail_load = !(success);   
...
{
  /* John and Kirsten driving here */
  struct thread *cur = thread_current ();
  struct list_elem *e; 
  struct thread *child;
   
  /* Search for child in list of children */
...
/* Helper function for process_wait(), returns the status of the child
   and removes the child from the children list. */
int 
found_child (struct thread *child)
{ 
  /* Kirsten and Matthew driving */
  int status;
...
process_exit (void)
{
  /* John, Kirsten, and Matthew driving */
  char *save_ptr = NULL; 
  int i;
  struct thread *cur = thread_current ();
  uint32_t *pd = NULL;
      ...
      if (&cur->parent != NULL) 
        {
          ...
          sema_down (&(cur->walking_dead));
          sema_up (&(cur->child_wait));
	      }

      char *name = strtok_r (cur->name, " ", &save_ptr); 
      printf ("%s: exit(%d)\n", name, cur->status_process);

      lock_acquire (&list_lock); 
      file_close (cur->file_opened);
      lock_release (&list_lock);

      /* Allow children to be able to exit */
      for (e = list_begin (&cur->children);  
           e != list_end (&cur->children); e = list_next (e)) 
        {
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  /* Deny writing to the file's own executable */
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. This method also pushes the arguments 
   onto the stack that are given with the file name and addresses
   to their location so that these arguments can be accessed. */
static bool
