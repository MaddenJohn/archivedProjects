    ...
    while not(searchNodes.isEmpty()):
        ...
        if (problem.isGoalState(new[0])):
            ...
            while (temp != startState):
                tempResult.insert(0, temp[1])
                temp = prevState[temp]
            return tempResult
        
        if (new[0] not in visited):
            visited.add(new[0])
            for state in problem.getSuccessors(new[0]):
                if not(prevState.has_key(state)):
                    prevState[state] = new
                searchNodes.push(state)
    return result
...
def uniformCostSearch(problem):
    ...
    startState = (problem.getStartState(), None, 0.0)
    prevCost[startState] = 0
    searchNodes.push(startState, 0)
    while not(searchNodes.isEmpty()):
        new = searchNodes.pop()
        tempResult = []
        if (problem.isGoalState(new[0])):
        ...
        if (new[0] not in visited):
            ...
            for state in problem.getSuccessors(new[0]):
                if (state[0] not in visited):
                    if not(prevState.has_key(state)):
                        prevState[state] = new
                        prevCost[state] = state[2] + prevCost[new]
                    searchNodes.push(state, state[2] + prevCost[new])
    return result
        ...
        if (new[0] not in visited):
            ...
            for state in problem.getSuccessors(new[0]):
                if not(prevState.has_key(state)):
                    prevState[state] = new
                    prevCost[state] = state[2] + prevCost[new]
                searchNodes.push(state, state[2] + prevCost[new] + heuristic(state[0], problem))
    return result
