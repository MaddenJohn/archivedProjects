...
def enhancedFeatureExtractorDigit(datum):
    ...
    I added one binary feature, which is whether a certain number has one or multiple regions, based on the pixels
    passed in
    ...
    "*** YOUR CODE HERE ***"
    #For instance in the digit data, consider the number of separate, connected regions of white pixels, 
    #which varies by digit type. 1, 2, 3, 5, 7 tend to have one contiguous region of white space while
    # the loops in 6, 8, 9 create more. 
    # initialize zeroPix
    zeroPix = {}
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) == 0 and not zeroPix[(x,y)] == 1:
                regions += 1
                checkZeroNeighbors(x, y, zeroPix, datum)
    # set features based on regions
    for x in range(1, 9):
        if regions > 1:
            features[x] = 1
        else:
...
# helper function to find a region of white space
def checkZeroNeighbors(x, y, zeroPix, datum):
    if x < DIGIT_DATUM_WIDTH and x >= 0 and y < DIGIT_DATUM_HEIGHT and y >= 0 and datum.getPixel(x, y) == 0 and not zeroPix[(x,y)] == 1:
        zeroPix[(x,y)] = 1
        checkZeroNeighbors(x - 1, y - 1, zeroPix, datum)
        checkZeroNeighbors(x, y - 1, zeroPix, datum)
        checkZeroNeighbors(x + 1, y - 1, zeroPix, datum)
        checkZeroNeighbors(x - 1, y, zeroPix, datum)
        checkZeroNeighbors(x + 1, y, zeroPix, datum)
        checkZeroNeighbors(x - 1, y + 1, zeroPix, datum)
        checkZeroNeighbors(x, y + 1, zeroPix, datum)
        checkZeroNeighbors(x + 1, y + 1, zeroPix, datum)
...
def enhancedPacmanFeatures(state, action):
    ...
    ghostDis = 999999
    for pos in ghostPos:
        ghostDis = min (ghostDis, manHattanDistance(pos, pacPos))
    
    # closest food   
    foodList = successor.getFood().asList()    
    foodDis = 999999
    for pos in foodList:
        foodDis = min (foodDis, manHattanDistance(pos, pacPos))
        
    
    featureCounter = util.Counter()
    featureCounter['closestGhost'] = 100.0 / (ghostDis + 1)
    featureCounter['closestFood'] = 1.0 / (foodDis + 1)
