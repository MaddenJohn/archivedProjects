...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # correct observation update
        allPossible = util.Counter()
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]
        # handles the "jail" edge case where the ghost is eaten and noisyDistance is None
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # loop through legalPositions and update them based on a possible new position. 
        # multiply beliefs by probability and normalize this to get new beliefs.
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Initialize based on legal positions the number of Particles required.
        while numPart > 0:
            for pos in self.legalPositions:
                listParticles.append(pos)
                numPart-= 1
                if (numPart <= 0):
                    break
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        particles = []
        # loop through and update a counter for new beliefs
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition) 
            if emissionModel[trueDistance] > 0:
        ...
        # resample the particles based on these beliefs or for other special cases
        if (noisyDistance == None):
                # Ate Ghost
    ...
    def elapseTime(self, gameState):
        ...
        allPossible = util.Counter()
        for p in self.legalPositions: allPossible[p] = 0.0
        # loop thorugh updating based on different combinations of potential locations
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += beliefs[oldPos] * prob
        allPossible.normalize()
        self.beliefs = allPossible
        # resample based on new beliefs
        particles = []
        if (allPossible.totalCount() == 0):
                self.initializeUniformly(gameState)
        else: 
            for _ in range(0, self.numParticles/5):
                particles.append(util.sample(allPossible))
            self.particles = particles
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        result = util.Counter()
        # loop through, using counter to get distribution
        for pos in self.legalPositions:
            count = self.particles.count(pos)
            result[pos] = count
        # case where ghost is in jail, simply set jail location
        result[self.getJailPosition()] = self.particles.count(self.getJailPosition())
        result.normalize()
        return result
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        allParts = self.legalPositions
        # get all posible combinations for the legalPositions
        for _ in range(self.numGhosts - 1):
            allParts = list(itertools.product(self.legalPositions,allParts))
        random.shuffle(allParts)
        listParticles = []
        numPart = self.numParticles
        # add these too list of particles until limit is reached
        while numPart > 0:
            for pos in allParts:
                listParticles.append(pos)
                numPart-= 1
    ...
    def observeState(self, gameState):
        ...
        allPossible = util.Counter()
        particles = []   
        # loop through each particle in beliefs, and each ghost for the particle, updating the belief of this
        for particle in belief:
            newBel = belief[particle]
            for x in range(self.numGhosts):
                if(noisyDistances[x] != None):
                    trueDistance = util.manhattanDistance(particle[x], pacmanPosition) 
                    if emissionModels[x][trueDistance] > 0:
                        newBel *= emissionModels[x][trueDistance] 
                    else:
                        newBel = 0
            allPossible[particle] = newBel
        allPossible.normalize()
        
        foundNoise = False
        i = 0
        full = 0
        # if a ghost is eaten, modify all particles to have the approprate representation of being in jail
        for noise in noisyDistances:
            if (noise == None):
                full += 1
                particles = []
                for particle in self.particles:
                    newParticle = self.getParticleWithGhostInJail(particle, i)
                    if (newParticle[1-i] != pacmanPosition):
        ...
        if not(foundNoise):
            # reinitialize if all particles recieve zero weight
            if (allPossible.totalCount() == 0):
                self.initializeParticles()
                particles = []
            else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            # update each of the entries in newParticle, using util.sample based on the probability of that position
            for i in range(self.numGhosts):
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        result = util.Counter()
        # count all different particles and normalize for distribution
