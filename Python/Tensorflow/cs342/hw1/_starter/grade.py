...
class BaseGrader:
	...
	def CHECK_SHAPE(self, t, s):
		...
		for a,b in zip(s,S):
			assert b is None or a == b, "Shape mismatch for tensor '%s' expected %s got %s"%(t.name, str(s), str(S))
	...
	def op_check(self):
		for o in self.g.get_operations():
			...
			if o.type == 'Const' and o.type in self.BLACK_LIST:
			...
			elif o.type in self.BLACK_LIST:
